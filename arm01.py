...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:#想要偵測得更具體可以提高信度
    ...
    # 打开摄像头
    cap = cv2.VideoCapture('IMG_5041.MOV')
    ...
    while cap.isOpened():
        ...
        try:
                ...
                #get coordinates
                shoulder = [landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x, landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]  # landmarks獲取座標(取左肩X值，取Y值)
        ...
        # #在左上角放置一個狀態框
        # cv2.rectangle(image,(0,0),(277,73),(245,117,16),-1)#畫方形(pic,左上點,右下點,顏色,粗細(使用填滿功能))
        ...
        # #顯示次數
        # cv2.putText(image,'REPS',(15,12),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),1,cv2.LINE_AA)#3:起始座標
